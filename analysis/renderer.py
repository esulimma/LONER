...
parser.add_argument("--get_matching_rgb", default=False, action="store_true", 
                    help="if set, for each rendered still, finds a matching RGB image from the rosbag and saves to the output folder")
    ...
    with torch.no_grad():
        ...
        if not args.no_render_stills:
            ...
            for kf in tqdm(poses_):
                ...
                timestamp_str = str(timestamp.item()).replace('.','_')[:5]
                rendered_timestamps.append(timestamp)
            ...
            if args.get_matching_rgb:
                ...
                bag = rosbag.Bag(pathlib.Path(full_config.dataset_path))

                camera = full_config.system.ros_names.camera
                lidar = full_config.system.ros_names.lidar
                camera_suffix = full_config.system.ros_names.camera_suffix
                topic_prefix = full_config.system.ros_names.topic_prefix
                image_topic = f"{topic_prefix}/{camera}/{camera_suffix}"
                
                bag_start = rospy.Time.from_sec(bag.get_start_time())

                image_times = []                
                for _, msg, ts in bag.read_messages(topics=image_topic):
                    msg_time = ts - bag_start
                    image_times.append(msg_time.to_sec())
                
                image_times = torch.Tensor(image_times).cpu()
                rendered_timestamps = torch.hstack(rendered_timestamps).cpu()

                closest_matches = image_times[torch.abs(rendered_timestamps.unsqueeze(0).cpu() - image_times.cpu()).argmin(dim=1)]

                render_idx = 0
                for _, msg, ts in bag.read_messages(topics=image_topic):
                    msg_time = (ts - bag_start).to_sec()
                    if msg_time - closest_matches[render_idx] < 0.01:
                        img = torch.from_numpy(bridge.imgmsg_to_cv2(msg, desired_encoding="rgba8"))
                        timestamp_str = str(rendered_timestamps[render_idx].item()).replace('.','_')[:5]
                        breakpoint()
                        save_img(img.permute(2,0,1), [], f"gt_image_{timestamp_str}.png", render_dir)
